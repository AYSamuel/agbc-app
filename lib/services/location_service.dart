  ...
  Future<LocationResult> validateAndNormalizeLocation(String location) async {
    ...
    try {
      ...
      if (locations.isNotEmpty) {
        ...
        if (placemarks.isNotEmpty) {
          ...
          String formattedLocation = formatLocation(
            ...
            place.country ?? (location.split(',').length > 1 ? location.split(',')[1].trim() : null)
          );
        ...
        if (locations.isNotEmpty) {
          ...
          if (placemarks.isNotEmpty) {
            ...
            String formattedLocation = formatLocation(
              ...
              place.country ?? (normalizedLocation.split(',').length > 1 ? normalizedLocation.split(',')[1].trim() : null)
            );
      ...
      if (location.contains(',')) {
        ...
        return LocationResult(
          isValid: true,
          normalizedLocation: normalizeLocation(location), // Just normalize the formatting
          originalInput: location,
        );
      }
      
      return LocationResult(isValid: false, error: 'Invalid location. Please enter a valid city and country');
    } catch (e) {
      ...
      return LocationResult(isValid: false, error: 'Error validating location: $e');
    }
  }
}
