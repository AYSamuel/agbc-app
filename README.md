...
**AGBC App** is a comprehensive church management tool designed to streamline administrative tasks and enhance member engagement. This Flutter-based application provides a centralized platform for managing church activities, member communication, and organizational workflows.
...
1.  **Clone the Repository:**
    ...
    cd agbc-app
    ```
    *(Replace `yourusername/agbc-app.git` with the actual repository URL if different)*
...
2.  **Configure Supabase:**
    ...
    *   **Database Schema & Policies:**
        ...
        *   *(Review these SQL files for any specific order or additional instructions.)*
    *   **Authentication:**
...
-   `supabase/`: Includes Supabase-specific configurations and backend code.
    ...
    -   `migrations/`: (If used) SQL files for database schema migrations, managed by the Supabase CLI.
        *(Note: The initial schema setup scripts like `rls_policies_tasks_table.sql` are currently in the root, but ongoing migrations might be placed here).*
...
*(Screenshots will be added soon to provide a visual overview of the application.)*
